...
(function (window, document) {
  ...
  /**
    ...
    * @private
    */
  const setHtml = function (el, text) {
    el.innerHTML = text;
  };
  ...
  /**
     ...
     * @private
     */
  const setValue = function (el, text) {
    el.value = text;
  };
  ...
  /**
     ...
     * @private
     */
  const setPlaceholder = function (el, text) {
  ...
  /**
     ...
     * @private
     */
  const computeNodes = function (elements, json) {
    let i = elements.length - 1;

    for (; i >= 0; i--) {
      const el = elements[i];
      const data = el.getAttribute('data-i18n').trim();
      const r = /(?:\s|^)(\w+):\s*(.*?)(?=\s+\w+:|$)/g; var
        m;
  ...
  function getAll(callback) {
    ...
    const locale = opts.localePreferred ? opts.localePreferred : opts.localeFallback;
  ...
  function get(key, callback) {
    ...
    if (opts.loadJson && jsonCache !== null) {
      if (!isArray) {
        ...
        const l = key.length; let i = l - 1; const
          obj = {};
